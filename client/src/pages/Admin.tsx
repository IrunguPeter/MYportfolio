...
import { ArrowUpRight, ClipboardList, ExternalLink, LayoutDashboard, Save, Settings2, Trash2 } from "lucide-react";
import { Link } from "wouter";
import { clearEnquiries, defaultSettings, getEnquiries, getSiteSettings, saveSiteSettings, type Enquiry, type SiteSettings } from "@/lib/siteSettings";
...
export default function Admin() {
  ...
  useEffect(() => setEnquiries(getEnquiries()), [tab]);
  ...
  const handleSave = () => { saveSiteSettings(settings); setSaved(true); window.setTimeout(() => setSaved(false), 2200); };
  const reset = () => { setSettings(defaultSettings); saveSiteSettings(defaultSettings); setSaved(true); };
      ...
      <main className="lg:pl-64">
        ...
        <div className="mx-auto max-w-6xl space-y-8 p-5 sm:p-8">
          {tab === "overview" && <>
            ...
            <section className="rounded-2xl border border-[#171717]/10 bg-white/75 p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">Quick start</p><h2 className="mt-2 font-display text-3xl font-medium">Keep the site fresh.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[#171717]/60">Update your contact details and starting prices here. Changes are saved in this browser immediately and appear when you view the public site on this device.</p></div><button onClick={() => setTab("settings")} className="inline-flex items-center gap-2 rounded-full bg-[#1747d1] px-4 py-3 text-xs font-semibold text-white">Edit settings <ArrowUpRight className="h-4 w-4" /></button></div></section>
          ...
          {tab === "enquiries" && <section className="rounded-2xl border border-[#171717]/10 bg-white/75"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#171717]/10 p-6"><div><p className="eyebrow">Lead inbox</p><h2 className="mt-2 font-display text-3xl font-medium">Project enquiries</h2></div><button onClick={() => { clearEnquiries(); setEnquiries([]); }} className="inline-flex items-center gap-2 rounded-full border border-red-200 px-4 py-2.5 text-xs font-semibold text-red-600 transition hover:bg-red-50"><Trash2 className="h-4 w-4" />Clear local inbox</button></div><EnquiryList enquiries={enquiries} /></section>}
        </div>
      </main>
    </div>
  );
